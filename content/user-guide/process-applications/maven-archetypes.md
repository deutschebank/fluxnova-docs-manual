...
They enable a quickstart for developing production-ready process applications using Fluxnova.
...
<table class="table table-bordered">
  ...
  <tbody>
    <tr>
      <td><a href="https://artifacts.camunda.com/artifactory/camunda-bpm/org/finos/fluxnova/bpm/archetype/camunda-archetype-cockpit-plugin/">Fluxnova Cockpit Plugin</a></td>
      <td>Plugin for Fluxnova Cockpit, contains REST-Backend, MyBatis database query, HTML and JavaScript frontend, Ant build script for one-click deployment</td>
    </tr>
    <tr>
      <td><a href="https://artifacts.camunda.com/artifactory/camunda-bpm/org/finos/fluxnova/bpm/archetype/camunda-archetype-ejb-war/">Process Application (EJB, WAR)</a></td>
      <td>Process application that uses a shared Fluxnova engine in a Java EE Container, e.g., Wildfly.
          Contains: Fluxnova EJB Client, Fluxnova CDI Integration, BPMN Process, Java Delegate as CDI bean, HTML5- & JSF-based start and task forms,
    ...
    <tr>
      <td><a href="https://artifacts.camunda.com/artifactory/camunda-bpm/org/finos/fluxnova/bpm/archetype/camunda-archetype-servlet-war/">Process Application (Servlet, WAR)</a></td>
      <td>Process application that uses a shared Fluxnova engine in a Servlet Container, e.g., Apache Tomcat.
    ...
    <tr>
      <td><a href="https://artifacts.camunda.com/artifactory/camunda-bpm/org/finos/fluxnova/bpm/archetype/camunda-archetype-spring-boot/">Fluxnova Spring Boot Application</a></td>
      <td>Application that uses the Fluxnova Spring Boot Starter.
          Contains: Spring Boot Process Application, Fluxnova Webapps, BPMN Process, Java Delegate, HTML5-based start and task forms,
          JUnit Test with in-memory engine, Maven Plugins for packing as an executable application.</td>
    </tr>
    <tr>
      <td><a href="https://artifacts.camunda.com/artifactory/camunda-bpm/org/finos/fluxnova/bpm/archetype/camunda-archetype-spring-boot-demo/">Fluxnova Spring Boot Application with Demo Users</a></td>
      <td>Same as the <i>Spring Boot Application</i> archetype and additionally creates demo users and groups for easy start with the Fluxnova Webapps (use <code>demo/demo</code> to log in).</td>
    </tr>
    <tr>
      <td><a href="https://artifacts.camunda.com/artifactory/camunda-bpm/org/finos/fluxnova/bpm/archetype/camunda-archetype-engine-plugin/">Process Engine Plugin</a></td>
...
We provide a template repository for every Fluxnova Archetype. 
Every repository contains a project generated from one specific template.
You can find the whole list on [GitHub](https://github.com/fluxnova?q=%22fluxnova-bpm-archetype-%22).

With every new release of the Archetypes, we will update those repositories with a new version as well.
This allows to investigate possible update paths from one Fluxnova version to another and also enables you to simply update your existing project by pulling in the latest changes.
...
4. Specify Maven coordinates and Fluxnova version and finish the project creation.
{{< img src="../img/eclipse-05-specify-maven-coordinates-and-fluxnova-version.png" title="Eclipse: Specify Maven coordinates and Fluxnova version" >}}
...
9. In the new project dialog, click on the left side on "Maven", check "Create from archetype" 
   and select any `org.finos.fluxnova.bpm.archetype` entry.
...
<pre class="console">
mvn archetype:generate -Dfilter=org.finos.fluxnova.bpm.archetype:
</pre>
...
mvn archetype:generate \
  ...
  -DarchetypeGroupId=org.finos.fluxnova.bpm.archetype \
  -DarchetypeArtifactId=fluxnova-archetype-ejb-war \
  -DarchetypeVersion=7.10.0 \
  -DgroupId=org.example.fluxnova.bpm \
  -DartifactId=fluxnova-bpm-ejb-project \
  -Dversion=0.0.1-SNAPSHOT \
  -Dpackage=org.example.fluxnova.bpm.ejb
</pre>


# Source Code and Customization

You can also customize the project templates for your own technology stack. Just [fork them on GitHub](https://github.com/finos/fluxnova-archetypes)!
