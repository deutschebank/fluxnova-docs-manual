...
In this section we briefly look at the contextual process execution model used by the Fluxnova CDI extension.
...
In Fluxnova CDI, a process instance can be associated with a CDI scope, the association representing a unit of work.
...
The Fluxnova CDI integration provides the `org.finos.fluxnova.bpm.engine.cdi.BusinessProcess` bean for controlling the association, most prominently:
...
Fluxnova CDI allows declaratively starting process instances and completing tasks using annotations. 
The `@org.finos.fluxnova.bpm.engine.cdi.annotation.StartProcess` annotation allows to start a process instance either by "key" or by "name". 
...
Depending on the configuration of the Fluxnova engine, the code of the annotated method and the starting of the process
instance will be combined in the same transaction. The `@org.finos.fluxnova.bpm.engine.cdi.annotation.CompleteTask`-annotation
...
Using Fluxnova CDI, the lifecycle of a bean can be bound to a process instance. To this extent, a custom context implementation is provided,
