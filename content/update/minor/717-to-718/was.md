...
The following steps describe how to update the Flowave artifacts on an IBM WebSphere application server 9 or
IBM WebSphere application server Liberty in a shared process engine setting. Throughout the procedure, refer 
to the [update guide][update-guide]. If not already done, download the [Flowave.18 IBM WebSphere distribution][was-distribution].
...
First, uninstall the Flowave web applications, namely the Flowave REST API (artifact name like `flowave-engine-rest`) 
and the Flowave applications Cockpit, Tasklist, and Admin (artifact name like `flowave-webapp`).

Uninstall the Flowave EAR; its name should be `flowave-ibm-websphere-ear-7.17.0-ee.ear`.

{{< note title="WebSphere Liberty steps" class="info" >}}
If you are using WebSphere Liberty, you just need to remove the Flowave EAR, REST API, and web applications from the
Liberty `$YOUR_SERVER/apps/` directory.
{{< /note >}}

# 2. Replace Flowave core libraries

With your first Flowave installation or update to 7.2, you have created a shared library named `Flowave`. We identify 
...
* `flowave-engine-7.17.0-ee.jar`
* `flowave-bpmn-model-7.17.0-ee.jar`
* `flowave-cmmn-model-7.17.0-ee.jar`
* `flowave-dmn-model-7.17.0-ee.jar`
* `flowave-xml-model-7.17.0-ee.jar`
* `flowave-engine-dmn-7.17.0-ee.jar`
* `flowave-engine-feel-api-7.17.0-ee.jar`
* `flowave-engine-feel-juel-7.17.0-ee.jar`
* `flowave-engine-feel-scala-7.17.0-ee.jar`
* `flowave-commons-logging-$COMMONS_VERSION.jar`
* `flowave-commons-typed-values-7.17.0-ee.jar`
...
Install the Flowave EAR, or the file `$WAS_DISTRIBUTION/modules/flowave-ibm-websphere-ear-7.18.0-ee.ear`.

* During the installation on WebSphere 9, the EAR will try to reference the `Flowave` shared library.
* On WebSphere Liberty, please follow [the EAR installation guide]({{< ref "/installation/full/was/manual-liberty.md#flowave-platform-ear" >}})
...
* On WebSphere 9:
  1. Deploy the web application `$WAS_DISTRIBUTION/webapps/flowave-engine-rest-7.18.0-ee-was.war` to your IBM WebSphere instance.
  2. Associate the web application with the `Flowave` shared library.
* On WebSphere Liberty:
  1. Place the web application `$WAS_DISTRIBUTION/webapps/flowave-engine-rest-7.18.0-ee-was.war` in the Liberty `$YOUR_SERVER/apps/` directory.
...
* On WebSphere 9:
  1. Deploy the web application `$WAS_DISTRIBUTION/webapps/flowave-webapp-ee-was-7.18.0-ee.war` to your IBM WebSphere instance.
  2. Associate the web application with the `Flowave` shared library.
* On WebSphere Liberty:
  1. Place the web application `$WAS_DISTRIBUTION/webapps/flowave-webapp-ee-was-7.18.0-ee.war` in the Liberty `$YOUR_SERVER/apps/` directory.
...
[was-distribution]: https://artifacts.camunda.com/artifactory/camunda-bpm-ee/org/finos/flowave/bpm/websphere/camunda-bpm-websphere9/7.18.0-ee/
