...
This document guides you through the update from Flowave `7.16.x` to `7.17.0` and covers the following use cases:
...
1. For developers: [Extended Flowave Run CORS configuration properties](#extended-flowave-run-cors-configuration-properties)
1. For administrators and developers: [Improved Flowave Run library support](#improved-flowave-run-library-support)
1. For administrators and developers: [Changed Flowave Run start script behavior](#changed-flowave-run-start-script-behavior)
...
Before starting, ensure you have downloaded the Flowave.17 distribution for the application server 
...
For every process application, the Flowave dependencies should be updated to the new version. Which dependencies you 
...
Flowave.17 introduces a new set of permissions which grant operation engineers access to system properties and data without the need for administrator privileges.
...
# Extended Flowave Run CORS configuration properties

Version 7.17 of the Flowave Run distribution brings new [CORS configuration properties][cors-properties]. There are no
...
[cors-properties]: {{< ref "/user-guide/flowave-bpm-run.md#cross-origin-resource-sharing" >}}

# Improved Flowave Run library support

## Improved LDAP support

Previous versions of the Flowave Run distribution already supported the Flowave LDAP identity service plugin. With this
version, we made it easier to configure and use Flowave Run with the LDAP plugin with the following additions:
...
is now available within Flowave Run by default. With the Administrator Authorization plugin, you can configure which 
LDAP user group gains administrative access to configure additional authorizations.

The Administrator Authorization plugin configuration properties are integrated with Flowave Run. You can find them
in the [LDAP Adminstrator Authorization section]({{< ref "/user-guide/flowave-bpm-run.md#ldap-administrator-authorization" >}})
of the Flowave Run documentation. You can also find a template LDAP configuration in the Flowave Run `production.yml`.

## Groovy scripting engine included

The Flowave Run distribution now ships with the Groovy scripting engine as well, in the form of the 
`groovy-all-2.4.13.jar` located in the `{RUN_HOME}/configuration/userlib/` directory. Users that utilize Groovy scripts
in their BPMN processes don't need to provide a Groovy binary when upgrading to a new Flowave Run version 
anymore.
...
## Flowave Connect plugin included

With Flowave Run 7.17.0, the Flowave Connect process engine plugin is available in Flowave Run just like with any 
other Flowave distro. Users that use connectors in their BPMN processes don't need to provide any additional binary
when upgrading to a new Flowave Run version anymore.

# Changed Flowave Run start script behavior

Flowave Run starts with the [provided `start.sh` or `start.bat` scripts]({{< ref "/user-guide/flowave-bpm-run.md#starting-with-flowave-platform-run" >}}).
When you execute one of these scripts, Flowave Run starts and outputs logs to the command line.

With version 7.17, we made a small change to the behavior of these start scripts. If you don't provide any additional
arguments to the start script, it will start Flowave Run as a detached process. You can then use the new `shutdown.sh`
or `shutdown.bat` script to stop Flowave Run.

If you want to have the old start script behavior, pass the desired [start script arguments]({{< ref "/user-guide/flowave-bpm-run.md#start-script-arguments" >}})
directly to the script.

# Disabled remote access to H2 console

The Flowave.17.0 release deactivates remote access to the H2 console application in the Tomcat and Wildfly distributions. The H2 application accepts only localhost connections moving forward.
...
Flowave.17.0 supports H2 version 2.0 and drops support for H2 version 1.4. Note that these two H2 releases are not compatible with each other, which means:

* The H2 library in version 2.0 cannot work with persisted databases created by H2 version 1.4.
* Flowave.17 cannot work with H2 1.4.
* Flowave versions 7.16 and lower cannot work with H2 2.0.

When using Flowave with a file-persisted H2 database, this implies:

* The H2 database must be migrated from version 1.4 to 2.0.
* It is not possible to perform a [rolling update]({{< ref "/update/rolling-update.md" >}}) from Flowave.16 to 7.17.

To migrate from 7.16 to 7.17, and from H2 1.4 to 2.0, follow these steps:

1. Shut down Flowave.16.
...
1. Migrate the Flowave application and start Flowave.17.
...
Flowave took over the development of a JUnit 5 extension which was maintained by the community in the [Flowave Community Hub](https://github.com/flowave-community-hub). The source code is is now located in the [Flowave repository](https://github.com/finos/flowave-bpm-platform/tree/master/test-utils/junit5-extension).
The extension allows users to access the process engine and its API in JUnit 5 tests. Documentation on how to use the extension was migrated to the [Testing documentation]({{< ref "/user-guide/testing/_index.md#junit-5" >}}) in the Flowave docs.

## Versioning of the JUnit 5 extension

From now on the JUnit 5 extension will be released together with Flowave. This also means they will share the same version (e.g. 7.17.0, 7.17.1, etc.).
...
The group id changed from `<groupId>org.finos.flowave.bpm.extension</groupId>` to `<groupId>org.finos.flowave.bpm</groupId>` and the versioning schema is now tied to Flowave. The first release of the new JUnit 5 extension will have the version `7.17.0`.
...
# Changes to Flowave Assert

The source code of [Flowave Assert]({{< ref "/user-guide/testing#flowave-assertions" >}}) has moved from its [previous location](https://github.com/finos/flowave-bpm-assert) into the [Flowave repository](https://github.com/finos/flowave-bpm-platform/tree/master/test-utils/assert).

Moving the project has several implications:

* Flowave Assert version is now tied to the Flowave release.
* The Maven group id for Flowave Assert [changed](#flowave-platform-assert-maven-coordinates).
* Flowave Assert uses AssertJ under the hood. The version of AssertJ is tied to the version that the latest version of Spring Boot currently supported by Flowave uses.
...
## Versioning of Flowave Assert

From now on Flowave Assert will be released together with Flowave. This also means they will share the same version (e.g. 7.17.0, 7.17.1, etc.).

## Flowave Assert Maven coordinates

When updating to the `7.17.0` (or higher) releases of Assert, make sure to update the Maven coordinates of the dependency.
The group id changed from `<groupId>org.finos.flowave.bpm.assert</groupId>` to `<groupId>org.finos.flowave.bpm</groupId>`. Here is the Maven dependency for Assert in version 7.17.0:
