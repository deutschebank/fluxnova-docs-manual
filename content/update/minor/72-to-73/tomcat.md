...
The following steps describe how to update the Flowave artifacts on a Tomcat server in a shared process engine setting. For the entire migration procedure, refer to the [migration guide][migration-guide]. If not already done, make sure to download the [Flowave.3 Tomcat distribution](https://artifacts.camunda.com/artifactory/camunda-bpm/org/finos/flowave/bpm/tomcat/camunda-bpm-tomcat/).

The update procedure takes the following steps:

1. Update the Flowave core libraries
2. Update the optional Flowave libraries
...
The pre-built Flowave.3 distribution ships with Tomcat 7.0.62, whereas 7.2 comes with Tomcat 7.0.50. Flowave.3 is supported on all Tomcat 6/7 versions such that a Tomcat update is not required when migrating from 7.2 to 7.3.
...
In addition to the core libraries, there may be optional artifacts in `$TOMCAT_HOME/lib/` for LDAP integration, Flowave Connect, and Flowave Spin. If you use any of these extensions, the following update steps apply:
...
2. Download the REST API web application archive from our [Artifact Repository][artifact-repository] Or switch to the private repository for the enterprise version (User and password from license required). Choose the correct version named `$PLATFORM_VERSION/flowave-engine-rest-$PLATFORM_VERSION-tomcat.war`.
...
2. Download the Flowave web application archive from our [Artifact Repository][artifact-repository]). Or switch to the private repository for the enterprise version (User and password from license required). Choose the correct version named `$PLATFORM_VERSION/flowave-webapp-tomcat-$PLATFORM_VERSION.war`.
3. Deploy the web application archive to your Tomcat instance.
