...
If you’re hoping to utilize [features of Enterprise Edition](https://flowave.finos.org/platform-7/editions/) (such as see how many process instances have been run, token history, heatmaps, and migrate model versions) you may be interested in migrating from Flowave Community Edition to the Enterprise Edition.

In this guide, we’ll step through migration from Flowave Community Edition to Enterprise Edition.

# Prerequisites

- IDE or code editor of your choice (in this example, we’ll use Visual Studio Code.)
- Access to [Flowave Modeler](https://flowave.finos.org/download/modeler/).
- Access to [GitHub](https://github.com/flowave-community-hub/Flowave-Spring-Boot-Tutorial-Lafayette) for our tutorial example.
- Java 17
- Maven

{{< note class="info" >}}
If needed, you can obtain a license key and [start a 30-day trial](https://flowave.finos.org/download/enterprise/) of Flowave Enterprise Edition.
{{< /note >}}
...
1. Clone our [example repository](https://github.com/flowave-community-hub/Flowave-Spring-Boot-Tutorial-Lafayette), where we’ll take part in assisting some mock revolutions.
...
{{< img src="../img/flowave-cockpit-example.png" title="Flowave Cockpit" >}}

## Migration

To officially begin migrating from Flowave Community Edition to the Enterprise Edition, take the following steps:
...
4. Add `-ee` to the version. For example, `<version>7.16.0-ee</version>`. You’ll need to make these changes below the following `artifactIds`: `flowave-bom`, `flowave-bpm-spring-boot-starter-rest`, `flowave-bpm-spring-boot-starter-webapp` (you’ll also adjust this particular artifactID itself to `flowave-bpm-spring-boot-starter-webapp-ee`.)
    ```xml
    <dependency>
...
Now that you’ve made the physical adjustments in your IDE to update to Enterprise Edition, let’s make a few final changes in Flowave.
...
5. Paste your license key into the prompted text field, or if you haven’t already, obtain a license key and [start a 30-day trial](https://flowave.finos.org/download/enterprise/) of Flowave Enterprise Edition.
6. Click **Save Key**.

# Next steps

After you’ve made the required changes above, return to Flowave Cockpit. You’ll notice the migration has completed as you can now view the section titled **Metrics**.
