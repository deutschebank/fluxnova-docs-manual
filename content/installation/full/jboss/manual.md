...
This document describes the installation of Flowave and its components on a vanilla [WildFly Application Server](http://www.wildfly.org) or JBoss EAP 7 / 8.
...
* `$WILDFLY_DISTRIBUTION` represents the downloaded pre-packaged Flowave distribution for WildFly, e.g. `flowave-bpm-wildfly-$PLATFORM_VERSION.zip` or `flowave-bpm-wildfly-$PLATFORM_VERSION.tar.gz`.
* `$PLATFORM_VERSION` denotes the version of Flowave you want to install or already have installed, e.g. `7.0.0`.
{{< /note >}}

## Setup

* For WildFly ≥27 / JBoss EAP 8, download the [Flowave WildFly distribution](https://downloads.camunda.cloud/release/camunda-bpm/wildfly/).
* For WildFly ≤26 / JBoss EAP 7, download the [`camunda-wildfly26-modules`](https://artifacts.camunda.com/artifactory/camunda-bpm/org/finos/flowave/bpm/wildfly/camunda-wildfly26-modules/).

### Copy Modules

Copy the modules from the `modules/` folder of the Flowave distribution, or extract the `flowave-wildfly-modules` archive, to the `$WILDFLY_HOME/modules/` of your WildFly application server.
...
The version shipped with Flowave is the version that the process engine is tested on and it is strongly recommended to use Flowave's version.
...
Add the Flowave subsystem as extension:

```xml
<server xmlns="urn:jboss:domain:20.0">
  <extensions>
    ...
    <extension module="org.finos.flowave.bpm.wildfly.flowave-wildfly-subsystem"/>
```

Configure the thread pool for the Flowave Job Executor:

Since Flowave.5, the configuration of the thread pool is done in the Flowave subsystem, not in the JBoss Threads subsystem anymore like it was done before 7.5.
...
<datasource jta="true" enabled="true" use-java-context="true" use-ccm="true"
  ...
  <connection-url>jdbc:h2:./flowave-h2-dbs/process-engine;DB_CLOSE_DELAY=-1;DB_CLOSE_ON_EXIT=FALSE</connection-url>
  <driver>h2</driver>
...
This section describes how to install optional dependencies. None of these are required to work with the core platform. Before continuing, make sure that Flowave is already installed according to [this step]({{< relref "#setup" >}}) for WildFly / JBoss EAP.
...
1. Download the Flowave web application that contains the web applications from our Maven Artifactory.
    Alternatively, switch to the private repository for the enterprise version (credentials from license required).
    * For [WildFly ≥27 / JBoss EAP 8](https://artifacts.camunda.com/artifactory/camunda-bpm/org/finos/flowave/bpm/webapp/camunda-webapp-wildfly/), the name of the artifact is `$PLATFORM_VERSION/camunda-webapp-wildfly-$PLATFORM_VERSION.war`.
    * For [WildFly ≤26 / JBoss EAP 7](https://artifacts.camunda.com/artifactory/camunda-bpm/org/finos/flowave/bpm/webapp/camunda-webapp-jboss/), the name of the artifact is `$PLATFORM_VERSION/camunda-webapp-jboss-$PLATFORM_VERSION.war`.
2. Optionally, you may change the context path to which the application will be deployed (default is `/flowave`).
...
4. Startup WildFly.
5. Access Cockpit, Tasklist, and Admin via `/flowave/app/cockpit`, `/flowave/app/tasklist` and `/flowave/app/admin`, or under the context path you configured.
...
1. Download the REST API web application archive from our Maven Artifactory.
   Alternatively, switch to the private repository for the enterprise version (credentials from license required).
    * For [WildFly ≥27 / JBoss EAP 8](https://artifacts.camunda.com/artifactory/public/org/finos/flowave/bpm/camunda-engine-rest-jakarta/), the name of the artifact is `$PLATFORM_VERSION/camunda-engine-rest-jakarta-$PLATFORM_VERSION-wildfly.war`.
    * For [WildFly ≤26 / JBoss EAP 7](https://artifacts.camunda.com/artifactory/public/org/finos/flowave/bpm/camunda-engine-rest/), the name of the artifact is `$PLATFORM_VERSION/camunda-engine-rest-$PLATFORM_VERSION-wildfly.war`.
...
To activate Flowave Connect functionality for a process engine, a process engine plugin has to be registered in `$WILDFLY_HOME/standalone/configuration/standalone.xml` as follows:
...
You can use the Flowave Spin plugin to extend the engine functionality to de-/serialize object variables from and to JSON and XML. For more information, see the [Spin Reference]({{< ref "/reference/spin/_index.md" >}}).
...
* `org/finos/flowave/spin/flowave-spin-dataformat-xml-dom-jakarta`
...
In order to activate Flowave Spin functionality for a process engine, a process engine plugin has to be registered in `$WILDFLY_HOME/standalone/configuration/standalone.xml` as follows:
...
The usage of Jackson annotations on WildFly together with the Flowave Spin JSON serialization can lead to problems.
...
This JAX-RS subsystem includes the Jackson library, the version of which does not match with the version used by the Flowave SPIN Plugin.
...
The Spin plugin also comes into play when JSON variables are set or read by the Flowave Process Engine.

See one of the following ways to fix this:

1. Change the Jackson `main` slot to the version which is used by the Flowave Spin Plugin.
 ...
 * Exclude the JAX-RS subsystem and add the Jackson dependencies, with the version which is used by the Flowave Spin Plugin.
 * This solution is also shown in the [Jackson Annotation Example for WildFly](https://github.com/finos/flowave-bpm-examples/blob/master/wildfly/jackson-annotations) in the Flowave example repository.
...
Flowave Spin is not available in scripts if a process definition is deployed via REST API. Because WildFly handles dependencies using its module system and Flowave engine module has no module dependency on the spin module.
