...
menu:
  main:
    ...
    pre: "Install and update the Flowave database schema used by the Process Engine."

---

This document will guide you through the installation and update of the Flowave [database schema]({{< ref "/user-guide/process-engine/database/database-schema.md" >}}) used by the process engine.
...
Consult the installation guide related to your setup to configure the database for a [remote engine]({{< ref "/installation/flowave-bpm-run.md" >}}), [shared engine]({{< ref "/installation/full/_index.md" >}}), or [embedded engine]({{< ref "/installation/spring-boot.md" >}}) accordingly.
...
Flowave supports a variety of databases that are documented in the [supported environments]({{< ref "/introduction/supported-environments.md#databases" >}}). 

Flowave supports the following ways of installing the database schema:

* Use the database migration tool [Liquibase](https://www.liquibase.org/) with the provided changelog for a semi-automatic installation and update. Liquibase keeps track of database schema changes. This allows you to focus on *when* changes should be applied rather than also on *which* changes are relevant. Starting with version `7.16.0`, Flowave comes with a curated *changelog file* that Liquibase can consume.
...
READ COMMITED is the required isolation level for database systems to run Flowave with. 
You may have to change the default setting on your database when installing Flowave. 
...
You can either install your database schema using Liquibase or using the provided SQL scripts manually. You can switch between those mechanisms when updating your Flowave version at a later stage if desired.
...
Flowave comes with a maintained changelog file that Liquibase can consume.
...
Open the `flowave-sql-scripts-$PLATFORM_VERSION/liquibase` folder to find the changelog. 
...
1. Run Liquibase's [update command](https://docs.liquibase.com/commands/community/update.html) referencing the `flowave-changelog.xml`.
...
To install the database schema required for Flowave, we provide a set of scripts with prepared DDL statements.
...
Open the `flowave-sql-scripts-$PLATFORM_VERSION/create` folder to find all available scripts. 
...
Updating to a newer Flowave minor version also requires a database schema update. You can reuse all the options available for a [schema installation](#installation) here as well.
...
In case you are just updating to a newer patch level of your Flowave installation, a schema update might not be necessary.
...
Open the `flowave-sql-scripts-$Y/liquibase` folder to find the changelog file.
...
1. Run Liquibase's [update command](https://docs.liquibase.com/commands/community/update.html) referencing the new `flowave-changelog.xml` of version `$Y`.
...
1. We highly recommend updating to the latest patch version that is within the bounds of the new minor version you are updating to (`$Y`).
    ...
    _Attention_: This step is only relevant when you are using an enterprise version of Flowave, e.g., `7.16.X` where `X > 0`.
...
1. Run Liquibase's [changelogSyncToTag command](https://docs.liquibase.com/commands/community/changelogsynctotag.html) referencing the `flowave-changelog.xml` and using your current database schema version as the tag.
You can pass on the connection details to your database instance via parameters as described in the Liquibase documentation or [create a properties file](https://docs.liquibase.com/workflows/liquibase-community/creating-config-properties.html).
...
Open the `flowave-sql-scripts-$Y/upgrade` folder to find all available scripts. 
...
Open the `flowave-sql-scripts-$Y/upgrade` folder to find all available patch scripts.
...
[Artifact Repository]: https://artifacts.camunda.com/artifactory/camunda-bpm/org/finos/flowave/bpm/distro/camunda-sql-scripts/
[pre-packaged distribution]: {{< ref "/introduction/downloading-flowave.md#full-distribution" >}}
