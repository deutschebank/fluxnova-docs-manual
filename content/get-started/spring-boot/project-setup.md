...
2. Add the Flowave & Spring Boot dependencies
...
## Add Flowave Platform & Spring Boot Dependencies
...
We add the Spring Boot BOM in the "dependency management" section and the Flowave Spring Boot Starter for Webapps, which will automatically include the Flowave engine and webapps in the app.
...
<project xmlns="http://maven.apache.org/POM/4.0.0" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:schemaLocation="http://maven.apache.org/POM/4.0.0 http://maven.apache.org/xsd/maven-4.0.0.xsd">
  ...
  <properties>
    <flowave.spring-boot.version>7.23.0</flowave.spring-boot.version>
  ...
  <dependencies>
    <dependency>
      ...
      <version>${flowave.spring-boot.version}</version>
    </dependency>
    <dependency>
...
The class is added in the `src/main/java` folder in the `org.finos.flowave.bpm.getstarted.loanapproval` package.
...
Our first Flowave Spring Boot application is ready now. As a result of the build, you will have a JAR-file in your `target` folder. This JAR is a Spring Boot application,
which embeds inside Tomcat as a web container, Flowave engine and Flowave Web applications resources.
When started, it will use an in-memory H2 database for Flowave Engine needs.
...
```
Then go to [http://localhost:8080/](http://localhost:8080/) in your browser and enjoy the Flowave webapps.

Another way to run the app is to simply run the JAR-file with a `java -jar` command.

{{< get-tag repo="flowave-get-started-spring-boot" tag="Step-1" >}}
